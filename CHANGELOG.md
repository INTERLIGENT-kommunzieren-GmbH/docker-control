...
## 2.7.2 — 2026-09-07
- **template/compose.yml**: the `cache` service now also answers to `valkey`, alongside the `redis` alias added in 2.7.0. All three names — the service name `cache` and both aliases — resolve to the same container on `backend-tier`.
- Add `module purge <vendor>/<name>` for a development checkout you are done with. `unlink` without `--purge` keeps the checkout in `htdocs/modules/` on purpose — it is your work, and the one thing Composer cannot reproduce — but until now nothing could see or remove it afterwards: `list` works from `htdocs/vendor/` and the `composer.json` repositories entries, so a checkout no path repository points at was invisible to every command, and the deletion path was reachable only from inside `unlink --purge`. The only way out was `rm -rf`.
- `module purge` is pure filesystem: the module is by definition not linked, so there is no `composer.json` entry to remove, no Composer call to make and nothing to roll back — which is also why it works with the stack down. It reports anything that exists only in the checkout (uncommitted changes, commits on no remote) before asking, and it confirms even a clean checkout, unlike `unlink --purge` where the flag is itself the opt-in. It refuses a module that is still linked and points at `unlink --purge`, since deleting the checkout out from under a live path repository would leave the application requiring a package nothing can resolve.
- `module list` now shows such a checkout as `stray`, without which `purge` would be undiscoverable.
- Fix `module link` always failing after a plain `unlink`. `unlink` ends with `composer update --prefer-source`, which reinstalls a real clone under `vendor/` while the checkout stays under `modules/` — so both exist as real directories, which is exactly the case `link` refused with "Remove whichever copy is stale before linking". Re-linking now keeps the `modules/` copy and discards the `vendor/` one, which Composer can always reinstall. Only when that vendor copy holds something git does not is it reported and confirmed rather than deleted, and declining still returns the old error as the escape hatch. `link` gains `-y/--yes` to skip that prompt, and a rollback after the discard says that `composer install` restores the vendor install.

## 2.7.1 — 2026-09-07
- An upgrade no longer leaves the ingress proxy serving the previous release's configuration. `upgrade` now stops the ingress before running `brew upgrade` and starts it again afterwards, and a plain `brew upgrade …/docker-control` is caught after the fact: `start`, `restart`, `status` and `status-ingress` report a proxy that is running an ingress stack other than the installed one, and `start`/`restart` offer to cycle it. The restart is never automatic — it briefly drops HTTPS for every project on the host, not just the one being started.
- The notice compares the *contents* of the ingress directory, not version numbers, so upgrades that don't touch `ingress/` stay silent. Nothing recorded is treated as "don't know" rather than "stale", so an existing installation is quiet until its next `start-ingress` establishes the baseline.
- There is deliberately no Homebrew-side hook for this. A formula's only hook is `post_install`, which runs *after* the new keg is linked — so it can never stop the proxy first — and runs sandboxed with no `$HOME` and no network, so it cannot reach the Docker socket or the `~/.docker` context at all. The stanzas that would give the wanted semantics (`preflight`/`uninstall_preflight`) are cask-only, and casks are macOS-only. The tool therefore notices for itself, which also covers `brew install`, `brew postinstall` and a hand-swapped binary. The tap and its generated formula are untouched.
- After a successful `brew upgrade`, the ingress is started by invoking the *newly installed* binary rather than in-process. The ingress assets are resolved relative to the running executable, and that is still the pre-upgrade keg — starting in-process would re-seed `etc/docker-control/ingress/volumes` from the old `share/docker-control/ingress` and start the old `compose.yml`, which is exactly the staleness being cleared. A failed `brew upgrade` brings the proxy back up before reporting the error, so an upgrade that doesn't happen can't leave the host without a proxy.
- `start-ingress` now creates `etc/docker-control/ingress/volumes` even when there is nothing to copy into it. Docker auto-creates a missing bind-mount source **as root**, which left that directory root-owned and unwritable by every later run. A Homebrew install always has the source and never hit this; a source build pointed at its own `DOCKER_CONTROL_INGRESS_DIR` did.

## 2.7.0 — 2026-09-07
- **template/compose.yml**: the `redis` service is now called `cache`, and keeps `redis` as a network alias on `backend-tier`. The name describes what the service is (Valkey, not Redis) and what it is used for, without requiring anything to be reconfigured: the alias means an application pointed at the hostname `redis` resolves to exactly the same container, so no `.env`, framework config or connection string has to change. The alias is a compatibility shim — once applications have moved to the `cache` hostname it can be dropped, which will be announced here as its own release.
- The host data directory stays `volumes/valkey/persist`, deliberately unrenamed. It holds the appendonly file, so renaming it would have started every existing project with a cold cache and dropped whatever was in it — sessions included. The inconsistency between the service name and the directory name is the cheaper of the two.
- `stop`, `restart` and `update` now pass `--remove-orphans` to `docker compose down`. Without it a renamed or removed service leaves its old container running: `down` stops only what the current `compose.yml` declares, so after this release a project would otherwise have had both `<project>-cache-1` and the old `<project>-redis-1` on `backend-tier`, both answering to the name `redis` and both appending to the same data directory. Orphan removal is general rather than specific to this rename — it applies to any service the template drops in future. Note that it removes containers carrying the project's compose labels that are no longer declared, so a container added to the project by hand outside `compose.yml` will now be cleaned up too.
- A project that has locally edited its `compose.yml` gets the usual conflict prompt for it (or a `compose.yml.dist` sidecar under `--yes`) and stays on the `redis` service name until the two are merged by hand. Projects that never touched the file receive the rename silently on the next `update`.
